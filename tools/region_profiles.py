...
NORTH_FIRE_GROUPS = {
    ...
    '포천·가평':   ['포천', '가평'],
    '파주·고양':   ['파주', '고양'],
    '남양주·구리': ['남양주', '구리'],   # 2026-08-17 추가 — 아래 주석 참조
}
# ⚠ 이 표는 관할 시군을 '하나도 빠짐없이' 담아야 한다.
#   2026-08-17 전체 점검에서 남양주·구리가 어느 권역에도 없어 두 관서의 산불위험이
#   화면에서 통째로 사라져 있던 것이 발견됐다(운영 data.js의 fire가 4건=8개 시군뿐).
#   수집단은 멀쩡했다 — NORTH_FIRE_SIGUN_MAP엔 남양주시·구리시가 있었는데
#   표시용 묶음에서만 빠져서, 값을 받아놓고 버리고 있었다.
#   tools/verify_profiles.py 가 '전 시군이 어느 권역엔가 속하는지'를 검사한다.
...
SOUTH_AWS_STATIONS = {
    ...
    '과천': [(116, '관악(레)'), (590, '과천')],
    # ⚠ 성남·평택은 '시군 전체'를 그대로 둔다. 경기 전체 사이트(소방서 34개)를 만들면서
    #   여기서 분당구·서탄면·송탄을 빼냈다가, 경기남부 사이트의 성남이 2곳→1곳,
    #   평택이 7곳→5곳으로 줄어 관측소를 잃었다(2026-08-19 회귀, 점검에서 발견).
    #   분당구에만 비가 와도 남부 화면엔 0mm로 뜨는 상태였다.
    #   → 시군 목록은 원상복구하고, 소방서 분리용 키를 '추가'로 둔다. 같은 관측소가
    #     두 키에 겹쳐도 각 키 안에서 최댓값을 고르는 방식이라 서로 간섭하지 않는다.
    #     (경기 전체의 성남소방서는 분당구 값까지 포함해 조금 넓게 보는데, 강수는
    #      넓게 보는 쪽이 안전하다. 분당소방서는 분당구만 정확히 본다.)
    '성남': [(364, '분당구'), (572, '성남')],
    '분당': [(364, '분당구')],
    ...
    '송탄': [(355, '서탄면'), (471, '송탄')],   # 송탄소방서 관할 = 평택 북부
    # ⚠ 고덕면(356)이 평택·송탄 중 어느 관할인지 미확정 — 점검에서 '송탄'이라는
    #   지적이 나왔으나 확증이 없어 지금은 평택에만 둔다. 사장님 확인 후 옮길 것.
}
SOUTH_AWS_FALLBACK = {}
